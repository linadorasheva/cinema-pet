<template>
  <article class="film-card">
    <h3 class="film-card__title" @click="openMoviePopup">{{ movieTitle }}</h3>
    ...
    <img :src="imgSource" :alt="movieTitle" class="film-card__poster" />
...
import MoviePopupContent from "@/components/MoviePopupContent/index.vue";
...
export default class FilmCard extends Vue {
  ...
  public get movieDescription(): string {
    ...
    return this.options.card.film_info.description.length < 140
      ? this.options.card.film_info.description
      : `${this.options.card.film_info.description.slice(0, 140)}...`;
  }
  ...
  public async openMoviePopup(): Promise<void> {
    const componentProps: IModalComponentProps = { options: this.options };
    const modalProps: IModalProps = {
      adaptive: true,
      height: "100%",
      width: "100%",
      classes: "v--modal-box--popup",
    };

    this.$modal.show(MoviePopupContent, componentProps, modalProps);
  }
}
</script>
